...
class BrandAdminTest(TestCase):
    def setUp(self):
        ...
        self.list = reverse('admin:labels_brand_changelist')
    ...
    def test_add(self):
        url = reverse('admin:labels_brand_add')
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)

    def test_detail(self):
        shipment = factories.BrandFactory()
        url = reverse('admin:labels_brand_change', args=(shipment.pk,))
        with self.assertNumQueries(5):
            response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_delete(self):
        shipment = factories.BrandFactory()
        url = reverse('admin:labels_brand_delete', args=(shipment.pk,))
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
